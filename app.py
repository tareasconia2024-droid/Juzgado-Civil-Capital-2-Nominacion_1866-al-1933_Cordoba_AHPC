...
app = Flask(__name__)
DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ahpc_civil2.db')
...
def index():
    ...
    rango = cur.fetchone()
    cur.execute("SELECT COUNT(DISTINCT causa) as t FROM registros WHERE causa IS NOT NULL AND causa != '' AND causa != 'sin datos'")
    causas = cur.fetchone()['t']
    cur.execute("""SELECT causa, COUNT(*) as t FROM registros
                   WHERE causa IS NOT NULL AND causa != '' AND causa != 'sin datos'
...
def estadisticas():
    ...
    cur.execute("""SELECT causa, COUNT(*) as total FROM registros
                   WHERE causa IS NOT NULL AND causa != '' AND causa != 'sin datos'
                   GROUP BY causa ORDER BY total DESC LIMIT 30""")
    top_causas = [dict(r) for r in cur.fetchall()]
    cur.execute("SELECT COUNT(*) as t FROM registros"); total = cur.fetchone()['t']
    cur.execute("SELECT COUNT(DISTINCT causa) as t FROM registros WHERE causa IS NOT NULL AND causa != ''"); total_causas = cur.fetchone()['t']
    cur.execute("SELECT COUNT(DISTINCT legajo) as t FROM registros WHERE legajo IS NOT NULL AND legajo != ''"); total_legajos = cur.fetchone()['t']
    conn.close()
...
def exportar_pdf():
    ...
    elements = [
        ...
        Paragraph("Juzgado Civil Capital 2° Nominación · Índice Onomástico de Expedientes (1883–1925)", sub),
        Spacer(1, .2*inch),
    ]
    data = [['Año', 'Legajo', 'Expte.', 'Partes', 'Asunto']]
    for r in rows:
        data.append([
            str(r['anio'] or ''),
            str(r['legajo'] or ''),
    ...
    t.setStyle(TableStyle([
        ...
        ('TEXTCOLOR',     (0,0), (-1,0), colors.HexColor('#e0eeff')),
        ('FONTNAME',      (0,0), (-1,0), 'Helvetica-Bold'),
        ('FONTSIZE',      (0,0), (-1,0), 8),
        ('ALIGN',         (0,0), (-1,-1), 'CENTER'),
        ('ALIGN',         (3,1), (4,-1), 'LEFT'),
        ('VALIGN',        (0,0), (-1,-1), 'MIDDLE'),
        ('GRID',          (0,0), (-1,-1), .5, colors.HexColor('#c0cfe0')),
    ...
    resp.headers['Content-Disposition'] = 'inline; filename=ahpc_civil2.pdf'
    return resp
